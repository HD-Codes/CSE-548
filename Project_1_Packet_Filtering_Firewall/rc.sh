...
# updated 05/12/2021								#
# updated 02/02/2022 - Hrishikesh Dahale		  				#
#################################################################################
#                                                                               #
#                                                                               #
...
$IPTABLES -A FORWARD -p UDP  -i $Client_NET_IFACE -o $Internet_IFACE  -d $Client_NET_IP --dport 80  -j ACCEPT

# Allows ping to 8.8.8.8
...
$IPTABLES -A INPUT -p UDP  --dport 80  -i $LO_IFACE         -d $WEB_IP_ADDRESS -j ACCEPT
$IPTABLES -A INPUT -p TCP  --dport 80  -i $Client_NET_IFACE -d $Client_NET_IP -j ACCEPT
$IPTABLES -A INPUT -p UDP  --dport 80  -i $Client_NET_IFACE -d $Client_NET_IP -j ACCEPT
...
$IPTABLES -A INPUT -p UDP  -i $Internet_IFACE --sport 80  -j ACCEPT
...
$IPTABLES -A OUTPUT -p UDP  --sport 80 -o $Client_NET_IFACE -j ACCEPT
...
$IPTABLES -A OUTPUT -p UDP  --dport 80 -o $Internet_IFACE  -j ACCEPT
...
$IPTABLES -t nat -A POSTROUTING -p udp -o $Internet_IFACE --dport 80 -j MASQUERADE
